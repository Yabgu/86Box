...
{                                                                               \
		...
		SEG_CHECK_WRITE(&cpu_state.seg_es);                             \
                ...
                temp = inb(DX);                                                 \
                writememb_n(es, DEST_REG, addr64, temp); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                                                                                \
...
{                                                                               \
		...
		SEG_CHECK_WRITE(&cpu_state.seg_es);                             \
                ...
                if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;                                   \
                temp = inw(DX);                                                 \
                writememw_n(es, DEST_REG, addr64a, temp); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                                                                                \
...
{                                                                               \
		...
		SEG_CHECK_WRITE(&cpu_state.seg_es);                             \
                ...
                if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;                                   \
                temp = inl(DX);                                                 \
                writememl_n(es, DEST_REG, addr64a, temp); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                                                                                \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                temp = readmemb(cpu_state.ea_seg->base, SRC_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                check_io_perm(DX);                                               \
                outb(DX, temp);                                                 \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                temp = readmemw(cpu_state.ea_seg->base, SRC_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;   \
                check_io_perm(DX);                                               \
                check_io_perm(DX+1);                                             \
                outw(DX, temp);                                                 \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                temp = readmeml(cpu_state.ea_seg->base, SRC_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;   \
                check_io_perm(DX);                                               \
                check_io_perm(DX+1);                                             \
                check_io_perm(DX+2);                                             \
                check_io_perm(DX+3);                                             \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;                                      \
                CHECK_WRITE_REP(&cpu_state.seg_es, DEST_REG, DEST_REG);         \
                do_mmut_wb(es, DEST_REG, &addr64_2);                            \
                if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;                                      \
                temp = readmemb_n(cpu_state.ea_seg->base, SRC_REG, addr64); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                writememb_n(es, DEST_REG, addr64_2, temp); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                                                                                \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;                                      \
                temp = readmemw_n(cpu_state.ea_seg->base, SRC_REG, addr64a); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                writememw_n(es, DEST_REG, addr64a_2, temp); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                                                                                \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;                                      \
                temp = readmeml_n(cpu_state.ea_seg->base, SRC_REG, addr64a); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                writememl_n(es, DEST_REG, addr64a_2, temp); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;    \
                                                                                \
...
{                                                                               \
        ...
        {                                                                       \
                CHECK_WRITE_REP(&cpu_state.seg_es, DEST_REG, DEST_REG);         \
                writememb(es, DEST_REG, AL); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;      \
                if (cpu_state.flags & D_FLAG) DEST_REG--;                       \
                else                DEST_REG++;                                 \
                CNT_REG--;                                                      \
                cycles -= is486 ? 4 : 5;                                        \
...
{                                                                               \
        ...
        {                                                                       \
                CHECK_WRITE_REP(&cpu_state.seg_es, DEST_REG, DEST_REG + 1UL);   \
                writememw(es, DEST_REG, AX); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;      \
                if (cpu_state.flags & D_FLAG) DEST_REG -= 2;                    \
                else                DEST_REG += 2;                              \
                CNT_REG--;                                                      \
                cycles -= is486 ? 4 : 5;                                        \
...
{                                                                               \
        ...
        {                                                                       \
                CHECK_WRITE_REP(&cpu_state.seg_es, DEST_REG, DEST_REG + 3UL);   \
                writememl(es, DEST_REG, EAX); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                if (cpu_state.flags & D_FLAG) DEST_REG -= 4;                    \
                else                DEST_REG += 4;                              \
                CNT_REG--;                                                      \
                cycles -= is486 ? 4 : 5;                                        \
...
{                                                                               \
        ...
        {                                                                       \
                CHECK_READ_REP(cpu_state.ea_seg, SRC_REG, SRC_REG);             \
                AL = readmemb(cpu_state.ea_seg->base, SRC_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;      \
                if (cpu_state.flags & D_FLAG) SRC_REG--;                       \
                else                SRC_REG++;                                  \
                CNT_REG--;                                                      \
                cycles -= is486 ? 4 : 5;                                        \
...
{                                                                               \
        ...
        {                                                                       \
                CHECK_READ_REP(cpu_state.ea_seg, SRC_REG, SRC_REG + 1UL);       \
                AX = readmemw(cpu_state.ea_seg->base, SRC_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;      \
                if (cpu_state.flags & D_FLAG) SRC_REG -= 2;                     \
                else                SRC_REG += 2;                               \
                CNT_REG--;                                                      \
                cycles -= is486 ? 4 : 5;                                        \
...
{                                                                               \
        ...
        {                                                                       \
                CHECK_READ_REP(cpu_state.ea_seg, SRC_REG, SRC_REG + 3UL);       \
                EAX = readmeml(cpu_state.ea_seg->base, SRC_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                if (cpu_state.flags & D_FLAG) SRC_REG -= 4;                     \
                else                SRC_REG += 4;                               \
                CNT_REG--;                                                      \
                cycles -= is486 ? 4 : 5;                                        \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                temp2 = readmemb_n(es, DEST_REG, addr64_2); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;   \
                if (uncached)                                                   \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                if (uncached)                                                   \
                        readlookup2[(uint32_t)(es+DEST_REG)>>12] = old_rl2;     \
                temp2 = readmemw_n(es, DEST_REG, addr64a_2); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;   \
                if (uncached)                                                   \
                        readlookup2[(uint32_t)(es+DEST_REG)>>12] = (uintptr_t) LOOKUP_INV; \
                                                                                \
                if (cpu_state.flags & D_FLAG) { DEST_REG -= 2; SRC_REG -= 2; }            \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                if (uncached)                                                   \
                        readlookup2[(uint32_t)(es+DEST_REG)>>12] = old_rl2;     \
                temp2 = readmeml_n(es, DEST_REG, addr64a_2); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;   \
                if (uncached)                                                   \
                        readlookup2[(uint32_t)(es+DEST_REG)>>12] = (uintptr_t) LOOKUP_INV; \
                                                                                \
                if (cpu_state.flags & D_FLAG) { DEST_REG -= 4; SRC_REG -= 4; }  \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                uint8_t temp = readmemb(es, DEST_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;\
                setsub8(AL, temp);                                              \
                tempz = (ZF_SET()) ? 1 : 0;                                     \
                if (cpu_state.flags & D_FLAG) DEST_REG--;                       \
                else                DEST_REG++;                                 \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                uint16_t temp = readmemw(es, DEST_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;\
                setsub16(AX, temp);                                             \
                tempz = (ZF_SET()) ? 1 : 0;                                     \
                if (cpu_state.flags & D_FLAG) DEST_REG -= 2;                    \
                else                DEST_REG += 2;                              \
...
{                                                                               \
        ...
        {                                                                       \
                ...
                uint32_t temp = readmeml(es, DEST_REG); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) break;\
                setsub32(EAX, temp);                                            \
                tempz = (ZF_SET()) ? 1 : 0;                                     \
                if (cpu_state.flags & D_FLAG) DEST_REG -= 4;                    \
                else                DEST_REG += 4;                              \
