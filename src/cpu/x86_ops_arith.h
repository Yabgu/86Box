#define OP_ARITH(name, operation, setflags, flagops, gettempc)   \
		...
		uint8_t src;									\
                ...
                {                                                                               \
                        ...
                        dst = geteab();                         if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                        src = getr8(cpu_reg);                                               \
                        seteab(operation);                              if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                        setflags ## 8 flagops;                                                  \
		...
		uint16_t src;									\
                ...
                {                                                                               \
						SEG_CHECK_WRITE(cpu_state.ea_seg);                                      \
                        dst = geteaw();                        if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                        src = cpu_state.regs[cpu_reg].w;                                   \
                        seteaw(operation);                              if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                        setflags ## 16 flagops;                                                 \
                        CLOCK_CYCLES(timing_mr);                                                \
		...
		uint32_t src;									\
                ...
                {                                                                               \
						SEG_CHECK_WRITE(cpu_state.ea_seg);                                      \
                        dst = geteal();                        if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                        src = cpu_state.regs[cpu_reg].l;                                   \
                        seteal(operation);                              if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                        setflags ## 32 flagops;                                                 \
                        CLOCK_CYCLES(timing_mr);                                               \
        ...
        {                                                                                       \
                ...
                dst = getr8(cpu_reg);                                                               \
                src = geteab(); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                setflags ## 8 flagops;                                                          \
        ...
        {                                                                                       \
                ...
                src = geteaw();                                 if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;             \
                setflags ## 16 flagops;                                                         \
        ...
        {                                                                                       \
                ...
                src = geteal();                                 if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;             \
                setflags ## 32 flagops;                                                         \
        ...
        {                                                                                       \
                uint32_t dst = EAX;                                                             \
                uint32_t src = getlong(); if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;                                   \
                if (gettempc) tempc = CF_SET() ? 1 : 0;                                         \
                setflags ## 32 flagops;                                                         \
                EAX = operation;                                                                \
                CLOCK_CYCLES(timing_rr);                                                        \
...
{
        uint32_t temp = getlong();                      if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;
        setznp32(EAX & temp);
        CLOCK_CYCLES(timing_rr);
        PREFETCH_RUN(timing_rr, 5, -1, 0,0,0,0, 0);
        return 0;
...
#define ARITH_MULTI(ea_width, flag_width)                                       \
        dst = getea ## ea_width();                      if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
        switch (rmdat&0x38)                                                     \
        {                                                                       \
                case 0x00: /*ADD ea, #*/                                        \
                setea ## ea_width(dst + src);           if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                setadd ## flag_width(dst, src);                                 \
                CLOCK_CYCLES((cpu_mod == 3) ? timing_rr : timing_mr);               \
                break;                                                          \
                case 0x08: /*OR ea, #*/                                         \
                dst |= src;                                                     \
                setea ## ea_width(dst);                 if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
                setznp ## flag_width(dst);                                      \
                CLOCK_CYCLES((cpu_mod == 3) ? timing_rr : timing_mr);               \
                break;                                                          \
                case 0x10: /*ADC ea, #*/                                        \
                tempc = CF_SET() ? 1 : 0;                                       \
                setea ## ea_width(dst + src + tempc);   if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;     \
...
static int op80_a16(uint32_t fetchdat)
{
        ...
        if (cpu_mod != 3)
                SEG_CHECK_WRITE(cpu_state.ea_seg);
        src = getbyte();                        if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;
        ARITH_MULTI(b, 8);
        if ((rmdat & 0x38) == 0x38) {
...
static int op81_w_a16(uint32_t fetchdat)
{
        ...
        if (cpu_mod != 3)
                SEG_CHECK_WRITE(cpu_state.ea_seg);
        src = getword();                        if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;
        ARITH_MULTI(w, 16);
        if ((rmdat & 0x38) == 0x38) {
...
static int op81_l_a16(uint32_t fetchdat)
{
        ...
        if (cpu_mod != 3)
                SEG_CHECK_WRITE(cpu_state.ea_seg);
        src = getlong();                        if (__builtin_expect(cpu_state.abrt != ABRT_NONE, false)) return 1;
        ARITH_MULTI(l, 32);
        if ((rmdat & 0x38) == 0x38) {
